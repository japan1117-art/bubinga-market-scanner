...
export function runDemoScan(direction: Direction): ScanResult {
  ...
  const candidates = names.map((name, index) => scoreAsset({ id: index + 1, name, payout: 95 - index }, candles(profiles[index], direction, 30), candles(profiles[index], direction, 60), direction))
